...
export async function generateBadgePdf(input: BadgePdfInput): Promise<Buffer> {
  const { code, badgeNumber, subscriber, event } = input;
  const isVip = input.type === "VIP";
  const eventDate = new Date(event.date).toLocaleDateString("fr-FR", {
    day: "numeric", month: "long", year: "numeric",
  });

  const qrBuffer = Buffer.from(input.qrData.split(",")[1], "base64");

  let logoBuffer: Buffer | null = null;
  if (event.logoUrl) {
    try {
      const res = await fetch(event.logoUrl);
  ...
  const accent = isVip ? "#FFD700" : (event.themeColor || "#E8C547");
  ...
  return new Promise<Buffer>((resolve, reject) => {
    ...
    doc.fillColor(accent, 1).font("Helvetica-Bold").fontSize(10)
      .text(`#${String(badgeNumber).padStart(3, "0")}`, CX, badgeNumY, {
    ...
    if (subscriber.jobTitle) {
      doc.fillColor(accent, 1).font("Helvetica-Bold").fontSize(9)
        .text(subscriber.jobTitle, CX + 8, textY, { width: CW - 16, align: "center", characterSpacing: 0.3 });
      textY += 13;
    }
    ...
    if (subscriber.company) {
      ...
      doc.fillColor(companyColor, 1).font("Helvetica").fontSize(9)
        .text(subscriber.company, CX + 8, textY, { width: CW - 16, align: "center" });
    }
    ...
    doc.fillColor(codeColor, 1).font("Helvetica").fontSize(7)
      .text(code.slice(0, 16), CX + 10, FTR_Y + 14, { width: CW / 2 - 10, align: "left", characterSpacing: 0.5 });
