            ...
            <div className="bg-white rounded-[24px] shadow-lg p-7 sm:p-8 border border-gray-100 animate-fade-in">
              ...
              {searchBy === "code" ? (
                ...
                <div>
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/10 outline-none transition-all text-sm" placeholder="john@example.com" />
                </div>
              )}
              <button onClick={searchBadge} disabled={loading || (searchBy === "code" ? !code : !email)} className="btn-primary w-full mt-5 py-3.5 text-sm flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none">
                {loading ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Searching...</> : <><Search className="w-4 h-4" /> Find My Badge</>}
              </button>
            </div>
          )}
            ...
            <div className="animate-scale-in">
              <div className="bg-white rounded-[24px] shadow-xl overflow-hidden border border-gray-100">
                <div className="relative bg-gradient-to-br from-secondary via-secondary-light to-accent p-8 text-center noise-overlay grid-pattern">
                  <div className="relative z-10">
                    <div className="inline-flex items-center gap-1.5 bg-white/10 rounded-full px-3 py-1 mb-3">
                      <Shield className="w-3 h-3 text-success" /><span className="text-white/80 text-xs font-medium">Verified Badge</span>
                    </div>
                    <h3 className="text-white text-xl font-bold">{badge.eventTitle}</h3>
                    <p className="text-gray-400 text-sm mt-1">{new Date(badge.eventDate).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
                  </div>
                </div>
                <div className="p-8 text-center">
                  ...
                  {badge.isScanned && <div className="mt-4 bg-success/5 text-success border border-success/20 px-4 py-2.5 rounded-xl text-sm font-medium inline-flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-success" /> Scanned</div>}
                </div>
                <div className="px-8 pb-8 space-y-3">
                  ...
                  <button onClick={() => { setBadge(null); setCode(""); setEmail(""); }} className="w-full bg-gray-50 hover:bg-gray-100 text-muted hover:text-secondary py-3.5 rounded-[10px] text-sm font-medium transition-colors">Search Again</button>
                </div>
              </div>
            </div>
          )}
